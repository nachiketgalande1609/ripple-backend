...
function initializeSocket(server, db) {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("sendMessage", async (data) => {
     ...
     AND sender_id NOT IN (
       SELECT muted_user_id FROM muted_users WHERE user_id = ?
            ...
            const [[{ isMuted }]] = await db.query(
              `SELECT COUNT(*) AS isMuted FROM muted_users 
    ...
    socket.on("messageRead", async (data) => {
      ...
      try {
        await db.query(
          `UPDATE messages SET is_read = TRUE, read_timestamp = CONVERT_TZ(NOW(), 'UTC', 'Asia/Kolkata') 
    ...
    socket.on("send-reaction", async (data) => {
      ...
      try {
        if (reaction === null || reaction === "") {
          await db.query(
            `UPDATE messages SET reactions = JSON_REMOVE(reactions, CONCAT('$."', ? , '"')) WHERE message_id = ?`,
        ...
        const [[message]] = await db.query(
          `SELECT receiver_id, sender_id FROM messages WHERE message_id = ?`,
          [messageId],
        );
        ...
        if (message) {
          ...
          const targetUserId =
            senderUserId === sender_id ? receiver_id : sender_id;
    ...
    socket.on("viewStory", async (data) => {
      ...
      try {
        const [[storyOwner]] = await db.query(
        ...
        const [existing] = await db.query(
          `SELECT * FROM story_views WHERE user_id = ? AND story_id = ?`,
          [user_id, story_id],
        );
    ...
    socket.on("callUser", (data) => {
      ...
      if (receiverSocketId) {
        io.to(receiverSocketId).emit("callReceived", {
          signal,
          from,
          callerUsername,
